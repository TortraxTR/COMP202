...
public class HashBasedWordCO {
			...
			for (String word : sentence.split("\\s+")) {
				...
				if(word.length() >= minWordLen) {
					...
					for(String ignored : ignoreList) {
						if(word.equals(ignored)) notIgnored = false;
					}
					if (notIgnored) uWords.put(word);
				}
			}
		}
		return uWords;
	...
	public void fillCoMat(String text, String[] keyWords) {
		...
		for (String sentence : sentences) {
			//HashMapDH<String, HashSet<Integer>> wordsInSentence = new HashMapDH<String, HashSet<Integer>>();

			String wordsInSentence[] = sentence.split("\\s+");

			int i = 0;
			
			for (String wordt : wordsInSentence) {
				if (words.contains(wordt)) {
					
					String[] window = new String[(2*windowSize)+1];
					
					for(int k = 0; k < window.length; k++) {

						if (k + i < windowSize || i + k - windowSize > wordsInSentence.length -1) {
							window[k] = null;
						} else {
							try {
								window[k] = wordsInSentence[i + k - windowSize];
						...
						for(String winWord : window){
							this.coMat.get(wordt).increment(winWord);
						}
