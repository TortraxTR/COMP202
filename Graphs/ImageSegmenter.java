...
public class ImageSegmenter {
	...
	public Image segmentImage(Image input, double epsilon) {
		...
		UndirectedUnweightedGraph<PixelCoordinate> graph = new UndirectedUnweightedGraph<PixelCoordinate>();
		// TODO: IMPLEMENT THIS
		for (int r = 0; r < input.getHeight()-1; ++r) {
			for (int c = 0; c < input.getWidth()-1; ++c) {
				// Get all possible neighbors of pixel at row r and column c for fun
				PixelCoordinate pc = new PixelCoordinate(r,c);
				graph.insertVertex(pc);
				PixelCoordinate[] n = new PixelCoordinate[4];
